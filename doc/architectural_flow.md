...
- **Database**: PostgreSQL in staging/production (currently Supabase-hosted, per `DB_HOST`/`DB_PORT` settings); SQLite for local development with no `DATABASE_URL`/`DB_HOST` set.
